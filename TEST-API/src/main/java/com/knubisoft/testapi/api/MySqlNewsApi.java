...
public interface MySqlNewsApi {

    @GetMapping("/news/{id}")
    @Operation(summary = "Get news by id", description = "Return news by id")
    @ApiResponse(responseCode = "200", description = "Success", content = {@Content(
            mediaType = MediaType.APPLICATION_JSON_VALUE,
            schema = @Schema(implementation = MysqlNews.class))}
    )
    ResponseEntity<MysqlNews> getNews(@Parameter(description = "Id of news", in = ParameterIn.PATH, required = true)
    ...
    @ApiResponse(responseCode = "200", description = "Success", content = {@Content(
            mediaType = MediaType.APPLICATION_JSON_VALUE,
            array = @ArraySchema(schema = @Schema(implementation = MysqlNews.class)))}
    )
    ResponseEntity<List<MysqlNews>> getNewsByName(
    ...
    ResponseEntity<List<MysqlNews>> getAllNews();
}
