...
@RequestMapping("/es")
public interface ESNewsApi {

    @GetMapping("/news/{id}")
    @Operation(summary = "Get news by id", description = "Return news by id")
    @ApiResponse(responseCode = "200", description = "Success", content = {@Content(
            mediaType = MediaType.APPLICATION_JSON_VALUE,
            schema = @Schema(implementation = ESNews.class))}
    )
    ResponseEntity<ESNews> getNews(@Parameter(description = "Id of news", in = ParameterIn.PATH, required = true)
    ...
    @ApiResponse(responseCode = "200", description = "Success", content = {@Content(
            mediaType = MediaType.APPLICATION_JSON_VALUE,
            array = @ArraySchema(schema = @Schema(implementation = ESNews.class)))}
    )
    ResponseEntity<List<ESNews>> getNewsByName(
    ...
    ResponseEntity<ESNews> createNews(@Parameter(description = "Data for news", required = true)
    ...
    ResponseEntity<Iterable<ESNews>> getAllNews();
}
