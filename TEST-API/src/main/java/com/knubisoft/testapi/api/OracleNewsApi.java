...
public interface OracleNewsApi {

    @GetMapping("/news/{id}")
    @Operation(summary = "Get news by id", description = "Return news by id")
    @ApiResponse(responseCode = "200", description = "Success", content = {@Content(
            mediaType = MediaType.APPLICATION_JSON_VALUE,
            schema = @Schema(implementation = OracleNews.class))}
    )
    ResponseEntity<OracleNews> getNews(@Parameter(description = "Id of news", in = ParameterIn.PATH, required = true)
    ...
    @ApiResponse(responseCode = "200", description = "Success", content = {@Content(
            mediaType = MediaType.APPLICATION_JSON_VALUE,
            array = @ArraySchema(schema = @Schema(implementation = OracleNews.class)))}
    )
    ResponseEntity<List<OracleNews>> getNewsByName(
    ...
    ResponseEntity<List<OracleNews>> getAllNews();
}
