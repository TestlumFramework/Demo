...
public interface ClickhouseNewsApi {

    @GetMapping("/news/{id}")
    @Operation(summary = "Get news by id", description = "Return news by id")
    @ApiResponse(responseCode = "200", description = "Success", content = {@Content(
            mediaType = MediaType.APPLICATION_JSON_VALUE,
            schema = @Schema(implementation = ClickhouseNews.class))}
    )
    ResponseEntity<ClickhouseNews> getNews(@Parameter(description = "Id of news", in = ParameterIn.PATH, required = true)
    ...
    @ApiResponse(responseCode = "200", description = "Success", content = {@Content(
            mediaType = MediaType.APPLICATION_JSON_VALUE,
            array = @ArraySchema(schema = @Schema(implementation = ClickhouseNews.class)))}
    )
    ResponseEntity<List<ClickhouseNews>> getNewsByName(
    ...
    ResponseEntity<List<ClickhouseNews>> getAllNews();
}
